...
def update_conflicts(conflicts, agents):
    ...
    while i < len(conflicts):
        ...
        if con.type == ConflictType.EDGE:
            p1 = con.edge.first()
            p2 = con.edge.second()
            group1, group2 = split_agents2(con, agents)
            if len(group1) > 1:
            ...
            if len(indexes) == 0 or (len(indexes) > 0 and not extended) or only_some_continuing:
                ...
                for i in range(len(con.path)):          # remove unnecessary position conflicts
                    ...
                    if key in pos_conflicts.keys():
                        ...
                        hi = 0
                        #print("pos con not found")  # this is ok i think 
                    if p_con != None:
...
def sorting_key1(conflict):
    if conflict.type == ConflictType.EDGE:
        return (conflict.time, 0)  # First priority: edge conflicts sorted by time
    elif conflict.type == ConflictType.PATH:
        time = conflict.time + 2 * len(conflict.path)
        return (time, 1)  # Second priority: path conflicts sorted by time
...
def reorder_conflicts(conflicts):
    sorted_conflicts = sorted(conflicts, key=sorting_key1)
...
def split_agents(edge_conflict, agents):
    ...
    group1 = [] # agents going p1 to p2
    group2 = [] # agents going p2 to p1
    for ag in edge_conflict.agents:
        agent_path = agents[ag].path 
        if agent_path.index(p1) < agent_path.index(p2):
            group1.append(ag)
        else:
            group2.append(ag)
    return group1, group2 

def split_agents2(edge_conflict, agents):
    p1 = edge_conflict.edge.first()
    p2 = edge_conflict.edge.second()
